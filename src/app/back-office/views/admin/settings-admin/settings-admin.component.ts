...
export class SettingsAdminComponent implements OnInit, AfterViewInit {
  ...
  saveTimeRange()
  {
    ...
    for (let i=0;i<tlist.length; i+=2)
    {
      ...
      if(date2<=date1) 
      {
        this.notificationService.showNotificationWithoutTimer('top', 'center', 'danger', 'pe-7s-close-circle', `\<b>Error !\</b>\<br>The time range N°${(i/2)+1} is not valid`);
        return;
      }
    ...
    .then((result:ResultStatut)=>{
      ...
      this.notificationService.showNotificationWithoutTimer('top', 'center', 'success', 'pe-7s-close-circle', `\<b>Success !\</b>\<br>the market opening times have been successfully saved`);
    })
